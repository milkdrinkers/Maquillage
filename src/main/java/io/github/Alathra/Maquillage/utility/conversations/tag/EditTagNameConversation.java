...
public class EditTagNameConversation {
    ...
    static Prompt editNameStringPrompt = new StringPrompt() {
        ...
        public @NotNull String getPromptText(@NotNull ConversationContext conversationContext) {
            ...
            player.sendMessage(ColorParser.of("The current name for " + tag.getTag() + player.getName() + " is " + currentName).build());
    ...
    static Prompt confirmPrompt = new FixedSetPrompt("YES", "NO", "yes", "no", "Yes", "No") {
        ...
        protected @Nullable Prompt acceptValidatedInput(@NotNull ConversationContext conversationContext, @NotNull String s) {
            ...
            if (s.equalsIgnoreCase("YES")) {
                boolean success = TagHandler.updateTag(tag.getTag(), tag.getPerm(), updatedName, tag.getIdentifier(), tag.getID());
                if (success) {
