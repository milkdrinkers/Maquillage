...
public class DatabaseConfig implements VersionedConfig {
    ...
    public @NotNull Map<Integer, Migration> migrations() {
        return Map.of(
            1, Migration.builder()
                .rename(NodePath.path("database", "advanced", "connection-pool", "max-pool-size"), "maxpoolsize")
                .rename(NodePath.path("database", "advanced", "connection-pool", "min-idle"), "minidle")
                .rename(NodePath.path("database", "advanced", "connection-pool", "max-lifetime"), "maxlifetime")
                .rename(NodePath.path("database", "advanced", "connection-pool", "keepalive-time"), "keepalivetime")
                .rename(NodePath.path("database", "advanced", "connection-pool", "connection-timeout"), "connectiontimeout")
                .rename(NodePath.path("messaging", "address"), "addresses")
                .withAction(NodePath.path("messaging", "type"), (path, node) -> {
                    final String type = node.getString();
                    if ("sql".equalsIgnoreCase(type))
                        node.set("database");
                    else if ("plugin".equalsIgnoreCase(type))
                        node.set("plugin_messaging");
                    return null;
                })
                .build()
        );
    ...
    public void validate() throws ConfigValidationException {
        if (messaging.pollingInterval >= messaging.cleanupInterval) {
            throw new ConfigValidationException(
                "messaging.polling-interval (" + messaging.pollingInterval + "ms) " + "must be less than messaging.cleanup-interval (" + messaging.cleanupInterval + "ms)"
            );
        }
    }
    ...
    public static class Database {
        ...
        public String tablePrefix = "maquillage_";
