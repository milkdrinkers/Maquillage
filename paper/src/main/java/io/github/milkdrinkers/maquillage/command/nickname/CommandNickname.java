...
public class CommandNickname {
                ...
                future.thenAccept(playerProfileRes -> {
                    ...
                    if (
                        !sender.getUniqueId().equals(player.getUniqueId()) &&
                            !PermissionUtil.playerHasPermission(sender, "maquillage.command.nick.set.other")
                    ) {
            ...
            .executes((sender, args) -> {
                final String name = args.getOptionalByClass("player", String.class)
                    .orElseThrow(
                        () -> CommandAPIPaper.failWithAdventureComponent(
                            Component.translatable("maquillage.commands.module.nickname.nickname.set.player-not-found")
                        )
                    );
                final CompletableFuture<Optional<PlayerProfile>> future = CompletableFuture.supplyAsync(() -> Queries.Nickname.fetchPlayerProfile(name));
                ...
                future.thenAccept(playerProfileRes -> {
                    if (playerProfileRes.isEmpty()) {
                        sender.sendMessage(Component.translatable("maquillage.commands.module.nickname.nickname.set.player-not-found"));
                        return;
                    }
    ...
    private static void setNickname(CommandSender sender, OfflinePlayer player, CommandArguments args) {
        if (sender instanceof Player senderPlayer && Cooldowns.has(senderPlayer, CooldownType.CommandNickname)) {
            sender.sendMessage(Component.translatable("maquillage.commands.module.nickname.nickname.set.cooldown"));
            return;
        }

        if (!(args.get("nick") instanceof String nick)) {
            sender.sendMessage(Component.translatable("maquillage.commands.module.nickname.nickname.set.no-nickname"));
            return;
        }

        if (nick.length() > Cfg.get().module.nickname.length) {
            sender.sendMessage(Component.translatable(
                "maquillage.commands.module.nickname.nickname.set.too-long",
                Argument.numeric("characters", Cfg.get().module.nickname.length)
            ));
            return;
        }

        if (nick.matches("[^a-zA-Z0-9_ ]")) {
            sender.sendMessage(Component.translatable("maquillage.commands.module.nickname.nickname.set.illegal-characters"));
            return;
        }
        ...
        if (onlinePlayer != null) {
            ...
            if (data == null) {
                sender.sendMessage(Component.translatable("maquillage.commands.module.error.player-not-loaded"));
                return;
            }
        }
        ...
        final String prefix = !Cfg.get().module.nickname.prefix.enabled ? "" : Cfg.get().module.nickname.prefix.string;
        ...
        sender.sendMessage(Component.translatable(
            "maquillage.commands.module.nickname.nickname.set.success",
            Argument.string("player", Objects.requireNonNull(player.getName())),
            Argument.string("nickname", nick)
        ));
        ...
        return new CommandAPICommand(command)
            ...
            .executes((sender, args) -> { // When sender is anything else
                final String name = args.getOptionalByClass("player", String.class)
                    .orElseThrow(
                        () -> CommandAPIPaper.failWithAdventureComponent(
                            Component.translatable("maquillage.commands.module.nickname.nickname.clear.player-not-found")
                        )
                    );
                final CompletableFuture<Optional<PlayerProfile>> future = CompletableFuture.supplyAsync(() -> Queries.Nickname.fetchPlayerProfile(name));
                ...
                future.thenAccept(playerProfileRes -> {
                    if (playerProfileRes.isEmpty()) {
                        sender.sendMessage(Component.translatable("maquillage.commands.module.nickname.nickname.clear.player-not-found"));
                        return;
                    }
    ...
    private static void clearNickname(CommandSender sender, OfflinePlayer player) {
        ...
        if (onlinePlayer != null) {
            ...
            if (data.getNickname().isEmpty()) {
                sender.sendMessage(Component.translatable("maquillage.commands.module.nickname.nickname.clear.no-nickname"));
                return;
            }
        }
        ...
        sender.sendMessage(Component.translatable(
            "maquillage.commands.module.nickname.nickname.clear.cleared",
            Argument.string("player", Objects.requireNonNull(player.getName()))
        ));
