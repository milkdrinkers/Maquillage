...
public class TagGui extends AbstractGui {
    ...
    public void open(Player p, boolean editorMode) {
        ...
        final PaginatedGui gui = dev.triumphteam.gui.guis.Gui.paginated()
            .title(editorMode ? translate("maquillage.gui.tag.title-editor", p) : translate("maquillage.gui.tag.title", p))
            .rows(6)
            .disableItemPlace()
            .disableItemSwap()
            .disableItemDrop()
    ...
    private record PopulateButtons(TagGui parentGui) {
        ...
        /**
            ...
            private void infoButton(PaginatedGui gui, Player p) {
                ...
                item.editMeta(SkullMeta.class, meta -> {
                    ...
                    meta.customName(
                        ColorParser.of(Translation.of("maquillage.gui.tag.clear-item.name"))
                            .papi(p)
                            .mini(p)
                    ...
                    meta.lore(
                        Translation.ofList("maquillage.gui.tag.clear-item.lore").stream()
            ...
            private void createButton(PaginatedGui gui, Player p) {
                ...
                createItem.editMeta(meta -> {
                    meta.customName(translate("maquillage.gui.tag.create-item.name", p));
                    meta.lore(translateList("maquillage.gui.tag.create-item.lore", p));
                });
                gui.setItem(1, 5, PaperItemBuilder.from(createItem).asGuiItem(event -> {
                    ...
                    TagEditor.create(
                        ...
                        (response, player, data) -> {
                            ...
                            final boolean success = TagHolder.getInstance().add(data.tag(), PermissionUtil.sanitizePermission(data.permission()), data.label(), (int) Math.floor(data.weight())) != -1;
                            if (success) {
                                p.playSound(SOUND_CLICK);
                                player.sendMessage(Component.translatable("maquillage.commands.module.tag.create.success"));
                                new TagGui().open(player, parentGui.isEditorMode());
                            } else {
                                p.playSound(SOUND_CLICK_FAIL);
                                player.sendMessage(Component.translatable("maquillage.commands.module.tag.create.failure"));
                            }
                        },
                        (response, player, data) -> {
                        },
    ...
    private record PopulateContent(TagGui parentGui) {
        ...
        /**
                ...
                TagHolder.getInstance().cacheGet()
                    ...
                    .forEachOrdered(tag -> {
                        ...
                        item.editMeta(meta -> {
                            ...
                            meta.customName(
                                ColorParser.of(Translation.of("maquillage.gui.tag.item.name"))
                                    .papi(p)
                                    .mini(p)
                            ...
                            meta.lore(
                                Translation.ofList(parentGui.isEditorMode() ? "maquillage.gui.tag.item.lore-editor" : "maquillage.gui.tag.item.lore").stream()
                                    .map(s -> ColorParser.of(s)
                                        .papi(p)
                                        .mini(p)
                                        .with("label", tag.getLabel())
                                        .with("preview_namecolor", namecolor)
                                        .with("preview_tag", tag.getTag())
                                        .with("active", isActive ? Component.translatable("maquillage.gui.tag.item.active.yes") : Component.translatable("maquillage.gui.tag.item.active.no"))
                                        .build()
                                    )
                                    .toList()
                            );
            ...
            private void onClickEdit(PaginatedGui gui, Player p, Tag tag) {
                ...
                TagEditor.edit(
                    ...
                    (response, player, data) -> {
                        ...
                        final boolean success = TagHolder.getInstance().update(data.tag(), PermissionUtil.sanitizePermission(data.permission()), data.label(), tag.getDatabaseId(), (int) Math.floor(data.weight()));
                        if (success) {
                            player.playSound(SOUND_SUCCESS);
                            new TagGui().open(player, parentGui.isEditorMode());
                        } else {
                            player.playSound(SOUND_FAIL);
                            player.sendMessage(Component.translatable("maquillage.commands.module.tag.edit.perm.failure"));
                        }
                    },
                    (response, player, data) -> {
                    },
            ...
            private void onClickClone(PaginatedGui gui, Player p, Tag tag) {
                ...
                if (!success) {
                    gui.close(p, false);
                    p.playSound(SOUND_FAIL);
                    p.sendMessage(Component.translatable("maquillage.commands.module.tag.create.failure"));
                    return;
                }
            ...
            private void onClickDelete(PaginatedGui gui, Player p, Tag tag) {
                ...
                if (!success) {
                    gui.close(p, false);
                    p.playSound(SOUND_FAIL);
                    p.sendMessage(Component.translatable("maquillage.commands.module.tag.delete.failure"));
                    return;
                }
