...
public class NameColorGui extends AbstractGui {
    ...
    public void open(Player p, boolean editorMode) {
        ...
        final PaginatedGui gui = dev.triumphteam.gui.guis.Gui.paginated()
            .title(editorMode ? translate("maquillage.gui.namecolor.title-editor", p) : translate("maquillage.gui.namecolor.title", p))
            .rows(6)
            .disableItemPlace()
            .disableItemSwap()
            .disableItemDrop()
    ...
    private record PopulateButtons(NameColorGui parentGui) {
        ...
        /**
            ...
            private void nextButton(PaginatedGui gui, Player p) {
                ...
                nextPage.editMeta(meta -> meta.customName(translate("maquillage.gui.next-page", p)));
            ...
            private void previousButton(PaginatedGui gui, Player p) {
                ...
                prevPage.editMeta(meta -> meta.customName(translate("maquillage.gui.previous-page", p)));
            ...
            private void infoButton(PaginatedGui gui, Player p) {
                ...
                item.editMeta(SkullMeta.class, meta -> {
                    ...
                    meta.customName(
                        ColorParser.of(Translation.of("maquillage.gui.namecolor.clear-item.name"))
                            .papi(p)
                            .mini(p)
                    ...
                    meta.lore(
                        Translation.ofList("maquillage.gui.namecolor.clear-item.lore").stream()
            ...
            private void createButton(PaginatedGui gui, Player p) {
                ...
                createItem.editMeta(meta -> {
                    meta.customName(translate("maquillage.gui.namecolor.create-item.name", p));
                    meta.lore(translateList("maquillage.gui.namecolor.create-item.lore", p));
                });
                gui.setItem(1, 5, PaperItemBuilder.from(createItem).asGuiItem(event -> {
                    ...
                    NameColorEditor.create(
                        ...
                        (response, player, data) -> {
                            ...
                            final boolean success = NameColorHolder.getInstance().add(data.color(), PermissionUtil.sanitizePermission(data.permission()), data.label(), (int) Math.floor(data.weight())) != -1;
                            if (success) {
                                p.playSound(SOUND_CLICK);
                                player.sendMessage(Component.translatable("maquillage.commands.module.namecolor.create.success"));
                                new NameColorGui().open(player, parentGui.isEditorMode());
                            } else {
                                p.playSound(SOUND_CLICK_FAIL);
                                player.sendMessage(Component.translatable("maquillage.commands.module.namecolor.create.failure"));
                            }
                        },
                        (response, player, data) -> {
                        },
    ...
    private record PopulateContent(NameColorGui parentGui) {
        ...
        /**
                ...
                NameColorHolder.getInstance().cacheGet()
                    ...
                    .forEachOrdered(color -> {
                        ...
                        item.editMeta(meta -> {
                            ...
                            meta.customName(
                                ColorParser.of(Translation.of("maquillage.gui.namecolor.item.name"))
                                    .papi(p)
                                    .mini(p)
                            ...
                            meta.lore(
                                Translation.ofList(parentGui.isEditorMode() ? "maquillage.gui.namecolor.item.lore-editor" : "maquillage.gui.namecolor.item.lore").stream()
                                    .map(s -> ColorParser.of(s)
                                        .papi(p)
                                        .mini(p)
                                        .with("label", color.getColor() + color.getLabel())
                                        .with("preview_namecolor", color.getColor() + p.getName())
                                        .with("preview_tag", tag)
                                        .with("active", isActive ? Component.translatable("maquillage.gui.namecolor.item.active.yes") : Component.translatable("maquillage.gui.namecolor.item.active.no"))
                                        .build()
                                    )
                                    .toList()
                            );
            ...
            private void onClickFail(PaginatedGui gui, Player p, NameColor color) {
                ...
                p.sendMessage(Component.translatable("maquillage.gui.on-cooldown"));
            }
            ...
            private void onClickEdit(PaginatedGui gui, Player p, NameColor color) {
                ...
                NameColorEditor.edit(
                    ...
                    (response, player, data) -> {
                        ...
                        final boolean success = TagHolder.getInstance().update(data.color(), PermissionUtil.sanitizePermission(data.permission()), data.label(), color.getDatabaseId(), (int) Math.floor(data.weight()));
                        if (success) {
                            player.playSound(SOUND_SUCCESS);
                            new NameColorGui().open(player, parentGui.isEditorMode());
                        } else {
                            player.playSound(SOUND_FAIL);
                            player.sendMessage(Component.translatable("maquillage.commands.module.namecolor.edit.perm.failure"));
                        }
                    },
                    (response, player, data) -> {
                    },
            ...
            private void onClickClone(PaginatedGui gui, Player p, NameColor color) {
                ...
                if (!success) {
                    gui.close(p, false);
                    p.playSound(SOUND_FAIL);
                    p.sendMessage(Component.translatable("maquillage.commands.module.namecolor.create.failure"));
                    return;
                }
            ...
            private void onClickDelete(PaginatedGui gui, Player p, NameColor color) {
                ...
                if (!success) {
                    gui.close(p, false);
                    p.playSound(SOUND_FAIL);
                    p.sendMessage(Component.translatable("maquillage.commands.module.namecolor.delete.failure"));
                    return;
                }
